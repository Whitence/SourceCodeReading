...
public final class ConfigurationPropertySources {
	 ...
	 */
	/**
	 * 将支撑连接到ConfigurationPropertySource指定的 Environment.使环境管理的每个PropertySource调用适应 ，ConfigurationPropertySource并允许使用 解决configuration property names经典PropertySourcesPropertyResolver调用。
	 * 附加的解析程序将动态跟踪基础 Environment 属性源中的任何添加或删除。
	 * 参数：
	 * 环境 – 源环境（必须是 的 ConfigurableEnvironment实例）
	 * 另请参阅：
	 * get(Environment)
	 */
	public static void attach(Environment environment) {
		// 断言environment是ConfigurableEnvironment的实例
		Assert.isInstanceOf(ConfigurableEnvironment.class, environment);
		// 获取environment的属性源
		MutablePropertySources sources = ((ConfigurableEnvironment) environment)
				.getPropertySources();
		// 获取ATTACHED_PROPERTY_SOURCE_NAME对应的PropertySource
		PropertySource<?> attached = sources.get(ATTACHED_PROPERTY_SOURCE_NAME);
		// 如果attached不为空并且attached的源不是sources
		...
		// 如果attached为空
		if (attached == null) {
			// 创建一个新的ConfigurationPropertySourcesPropertySource
